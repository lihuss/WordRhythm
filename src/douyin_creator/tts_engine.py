...
class TTSEngine:
    """Generate per-segment narration clips with pluggable backends."""
    ...
    def synthesize_segments(self, segments: list[str], output_dir: Path) -> list[Path]:
        ...
        if self.backend == "gtts":
            return self._synthesize_all_gtts(normalized_segments, output_dir)
        if self.backend == "edge-tts":
            return asyncio.run(self._synthesize_all_edge_tts(normalized_segments, output_dir))
    ...
    def _load_xtts_model(cls, model_path: str, device: str):
        ...
        try:
            ...
            raise RuntimeError(
                "Missing dependency TTS for XTTS backend. Install with: pip install TTS"
            ) from exc
        ...
        if model_path_obj.exists() and model_path_obj.is_dir():
            ...
            _xtts_model = TTS("tts_models/multilingual/multi-dataset/xtts_v2").to(resolved_device)
        return _xtts_model

    async def _synthesize_all_edge_tts(self, segments: list[str], output_dir: Path) -> list[Path]:
        try:
            import edge_tts
        ...
        generated_paths: list[Path] = []
        for index, text in enumerate(segments):
            output_path = output_dir / f"{index:04d}.mp3"
            await self._synthesize_edge_tts_with_retry(edge_tts, text, output_path)
            generated_paths.append(output_path)
        return generated_paths

    def _synthesize_all_gtts(self, segments: list[str], output_dir: Path) -> list[Path]:
        try:
        ...
        for index, text in enumerate(segments):
            output_path = output_dir / f"{index:04d}.mp3"
            self._synthesize_gtts_with_retry(gTTS, text, output_path)
            generated_paths.append(output_path)
        return generated_paths

    def _synthesize_gtts_with_retry(self, gtts_class, text: str, output_path: Path) -> None:
        if output_path.exists():
            output_path.unlink()
        ...
        for _ in range(self.retries):
            try:
                tts = gtts_class(text=spoken_text, lang=self.language, slow=self.slow)
                tts.save(str(output_path))
                return
            except Exception as exc:  # pragma: no cover - network/runtime dependent
                last_error = exc

        raise RuntimeError(f"gTTS failed for text: {spoken_text}") from last_error

    async def _synthesize_edge_tts_with_retry(self, edge_tts_module, text: str, output_path: Path) -> None:
        if output_path.exists():
            output_path.unlink()
        ...
        raise RuntimeError(f"Edge-TTS failed for text: {text}") from last_error
